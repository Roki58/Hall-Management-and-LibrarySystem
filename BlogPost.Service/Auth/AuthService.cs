...
namespace BlogPost.Service.Auth
{
    public class AuthService : IAuthService
    {
        ...
        private async Task ValidateRequest(UserRequest userRequest)
        {
            ...
            UserLoginResponse response2 = await GetUserByNumber(userRequest.MobileNumber);
            UserLoginResponse response3 = await GetUserById(userRequest.StudentId);


            if (response != null || response2 != null || response3 != null)
            {
                throw new UserRegistrationException("Email | Mobile | ID already exist", new()
                {
                    { "Email | Mobile | ID", "already exist" }
                });
            }
        }
        ...
        public async Task<UserLoginResponse> GetUserById(string id)
        {
            User user = await _authRepository.userLoginByNumberAsync(id);
            UserLoginResponse response = _mapper.Map<UserLoginResponse>(user);
            return response;
        }
        public async Task<UserLoginResponse> GetUserByNumber(string number)
        {
            User user = await _authRepository.userLoginByNumberAsync(number);
            UserLoginResponse response = _mapper.Map<UserLoginResponse>(user);
            return response;
        }
